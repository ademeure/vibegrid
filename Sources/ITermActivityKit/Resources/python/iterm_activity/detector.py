    ...
    def resolve(self, entries: list[PollEntry], now: float | None = None, active_hold_override: float | None = None) -> dict[str, ResolvedActivity]:
        ...
        for entry in entries:
            ...
            if recent_active_rule_match and self._should_trigger_visible_active_rule(
                ...
                hold_seconds = (
                    max(THINKING_HOLD_SECONDS, active_hold_override if active_hold_override is not None else 0)
                    if recent_active_rule_match.id == "thinking-effort"
                    else (active_hold_override if active_hold_override is not None else profile.activity_tuning.active_rule_hold_seconds)
                )
                ...
                if (
                    ...
                    next_state.active_hold = ActiveHold(
                        ...
                        detail=detail,
                        expires_at=now + (active_hold_override if active_hold_override is not None else profile.activity_tuning.meaningful_change_hold_seconds),
                    )
                    status = "active"
                    reason = "meaningful-body-change"
                else:
