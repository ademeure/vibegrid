...
struct YAMLConfigCodec {
            ...
            case 1:
                ...
                if topLevelSection == "settings" {
                    ...
                    default:
                        ...
                        throw ConfigParseError.invalidLine(line.number, "Unknown settings key: \(key)")
                    }
    ...
    static func encode(_ config: AppConfig) -> String {
        ...
        lines.append("  defaultGridColumns: \(normalized.settings.defaultGridColumns)")
        lines.append("  defaultGridRows: \(normalized.settings.defaultGridRows)")
        lines.append("  gap: \(normalized.settings.gap)")
        lines.append("  defaultCycleDisplaysOnWrap: \(normalized.settings.defaultCycleDisplaysOnWrap ? "true" : "false")")
        lines.append("  animationDuration: \(formatDouble(normalized.settings.animationDuration))")
        lines.append("  controlCenterScale: \(formatDouble(normalized.settings.controlCenterScale))")
        lines.append("  largerFonts: \(normalized.settings.largerFonts ? "true" : "false")")
        lines.append("  themeMode: \(normalized.settings.themeMode.rawValue)")
        lines.append("  moveEverythingMoveOnSelection: \(normalized.settings.moveEverythingMoveOnSelection.rawValue)")
        lines.append("  moveEverythingCenterWidthPercent: \(formatDouble(normalized.settings.moveEverythingCenterWidthPercent))")
        lines.append("  moveEverythingCenterHeightPercent: \(formatDouble(normalized.settings.moveEverythingCenterHeightPercent))")
        lines.append("  moveEverythingOverlayMode: \(normalized.settings.moveEverythingOverlayMode.rawValue)")
        ...
        lines.append(
            "  moveEverythingCloseHideHotkeysOutsideMode: \(normalized.settings.moveEverythingCloseHideHotkeysOutsideMode ? "true" : "false")"
        )
